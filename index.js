...
(function() {
  ...
  d3.json('data.json', function(error, data) {
    var all_gists, all_items, cv_data, en_entries, en_gists, en_items, entries, gists, gists_node, info, items, max, min;
    if (error) {
      console.warn(error);
    }
    ...
    cv_data = {...data};
    cv_data.nodes = data.nodes.filter(function(d) {
      return d.type !== 'gist';
    });
    ...
    items = info.selectAll('.item').data(data.nodes.filter(function(d) {
      return d.type !== 'gist';
    }));
    ...
    en_items.append('div').attrs({
      ...
      switch (d.type) {
        ...
        case 'Education':
          return `@ ${d.location}`;
        case 'Experience':
          return `${d.location != null ? `@ ${d.location}, ${d.place}` : ""}
${d.technologies != null ? `<br />> ${d.technologies.join(' - ')}` : ""}`;
        case 'Project':
...
${d.thesis !== void 0 ? '<a href="data/' + d.thesis + '">Thesis</a>' : ''}`;
    ...
    gists = gists_node.selectAll('.gist').data(data.nodes.filter(function(d) {
      return d.type === 'gist';
    }));
    en_gists = gists.enter().append('div').attrs({
      id: function(d, i) {
        return `gist${i}`;
      },
      class: 'gist'
    ...
    en_gists.append('div').attrs({
      class: 'title'
    }).text(function(d) {
      return d.label;
    });
    en_gists.append('a').attrs({
      class: 'img',
      href: function(d) {
        return `${d.link}`;
      }
    }).html(function(d) {
      return `<img src='img/${d.img}'>`;
    });
    return en_gists.append('a').attrs({
      class: 'code',
      href: function(d) {
        return `${d.code}`;
      }
